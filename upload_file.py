...
def delete_drive_service_file(service, file_id):
    ...
    try:
        connection = psycopg2.connect(database = "smartmeeting",
                                      user = "root",
                                      password = "royliao6311",
                                      host = "140.136.155.145",
                                      port = "5432"
                                      )#連線資料庫
        cursor = connection.cursor()
        cursor.execute("DELETE FROM audiotext WHERE at_id=%s", (file_id, ))#需要有逗點
        connection.commit()
    ...
    if is_update_file_function is True:
        ...
        if update_drive_service_name is None:  # 上傳資料夾內的所有檔案會跑這裡
            ...
            for i in range(len(UploadFilesPath)):
                name,id=update_file(service=service, update_drive_service_name=UploadFilesName[i],
                            local_file_path=UploadFilesPath[i], update_drive_service_folder_id=get_folder_id)
                print(name)
                print(id)
                finish_file.append(name)
            print("=====上傳檔案完成=====")
            #print(finish_file)
            fileTest = ("C:/Users/Admin/Desktop/專題/上傳雲端/上傳資料/UploadFiles/"+finish_file[1])
            #print(fileTest)
            shutil.rmtree('UploadFiles')
...
if __name__ == '__main__':
    print(os.getcwd())
    main(is_update_file_function=bool(True), update_drive_service_folder_name='Uploaded Files', update_drive_service_name=None, update_file_path=os.getcwd() + '/UploadFiles/')
